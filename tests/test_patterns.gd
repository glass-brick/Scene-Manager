...
func test_every_shipped_pattern_loads():
	var names = ["circle", "curtains", "diagonal", "horizontal", "radial", "scribbles", "squares", "vertical"]
	for name in names:
		assert_true(_manager._load_pattern(name) is Texture, "%s failed to load" % name)
